import type { Point } from '@re-headless/protocol';
import { HashSet } from './hash-set.js';
import { Heap } from './heap.js';
import { Node } from './node.js';
import type { NodeUpdate } from './node-update.js';
...
export class Pathfinder {
  ...
  private simplifyPath(path: Node[]): Point[] {
      ...
      if (direction.x !== lastDirection.x || direction.y !== lastDirection.y) {
        ...
        if ((direction.x & direction.y) !== 0) {
          ...
          if (direction.x !== lastDirection.x) {
            ...
            waypoints.push({ x: path[i].x, y: path[i - 1].y });
          }
        } else {
    ...
    waypoints.push({ x: path[path.length - 1].x, y: path[path.length - 1].y });
    return waypoints;
  }

  private retracePath(start: Node, end: Node): Point[] {
    const path: Node[] = [];
    let currentNode: Node | null = end;
